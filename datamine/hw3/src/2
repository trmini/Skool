...
char *netname;
{
  ...
  if ((net = bpnn_read(netname)) == NULL) {
    if (train_n > 0) {
      ...
      /* bthom ===========================
	make a net with:
	  imgsize inputs, 20 hiden units, and 20 output units
          */
...
double *err;
{
  ...
  for (index = 1; index < 21; index++) {
    delta[index] = net->target[index] - net->output_units[index];
    *err = (0.5 * delta[index] * delta[index]);

    /*** If the target unit is on... ***/
    if (net->target[index] > 0.5) {
