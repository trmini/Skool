...
static int tell_status(int fdout, int type_of_mess);
...
int main(int argc, char **argv) {
  ...
  if (sema_init(&my_lock, 1, 1) == -1) {
    fprintf(stderr, "Could not initialize my lock semaphore\n");
    exit(1);
  }
  ...
  /* Getting dir or request info first */
  i = 0;
  ...
  /* Handle request send status */
  if (!strcmp(buff,"send status")) {
    fdout = fdin;
    tell_status(fdout, 1);
    tell_status(fdout, 0);
    exit(1);
...
static int tell_status(int fdout, int type_of_mess) {
  ...
  switch(type_of_mess) {
    case 0:  /* Send out transinfo */
      buff[0] = '0';
      u_write(fdout, buff, 1);
      while (f_curr != NULL && f_curr->next != NULL) {
        sprintf(buff,"%s %ld %ld %ld %ld %s ", f_curr->f_name, f_curr->f_size, f_curr->o_size,
                                             f_curr->f_time, f_curr->o_time, f_curr->action);
        buff_p = buff;
        size = strlen(buff);
        while ( (size > 0) && (n = u_write(fdout, buff_p, size)) != size) {
          if (n < 0 && (n = 0 || errno != EINTR) ) {
            return 0;
          }
          size -= n;
    ...
    case 1: /* Send out recordinfo */
      buff[0] = '1';
      sprintf(buff+1, "%d %d %d %ld %ld ", section.set_num, section.file_num, section.send_num, section.byte_num, section.last_time);
      buff_p = buff;
      size = strlen(buff);
      while ( (size > 0) && (n = u_write(fdout, buff_p, size)) != size) {
        if (n < 0 && (n = 0 || errno != EINTR) ) {
