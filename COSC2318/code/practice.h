#include "c:\mydocu~1\cosc\code\rational.h"

const int LIMIT=49;

//Display the prompt for user to choose between go on or exit
BOOLEAN prompt()
 {
  char key;
  window(1,1,80,25);
...
//Display the check window if the user input correct answer or not
BOOLEAN check()
 {
  char answer;
  char buffer[4096];
...
//Display warning when user input improper data
void warning()
 {
  char buffer[4096];
  gettext(1,1,80,25,buffer);
...
//Actually, this is a small program which user can practice with
